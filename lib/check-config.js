...
var isDNSSetup = function(loopback_ip) {
  Logger.info("Checking DNS setup")
...
exports.setup = function (loopback_ip) {
  ...
  if (hasBoot) {
    ...
    if (isDNSSetup(loopback_ip) === true) {
      return true
    }
    ...
    if (updateDNSStatus.status !== 0) {
      return Logger.warn("Error while updating boot2docker DNS: " + updateDNSStatus.stderr)
    }
