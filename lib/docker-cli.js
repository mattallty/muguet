...
DockerCLI.prototype.getRunningVMName = function () {
  ...
  var result = this.execAsNormalUser("docker-machine ls | tail -2 | tr -s ' ' | cut -d ' ' -f 1,3").stdout,
      vmName = null

  result.split('\n').some(function (line) {
    var parts = line.split(' ')
    return (parts[1] && parts[1] === 'Running') ? (vmName = parts[0]) : false
  })

  return (this.runningVM = vmName)
}
