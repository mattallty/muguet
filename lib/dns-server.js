...
DNSServer.prototype.setEntries = function (entries) {
  ...
  this.entries = {}
  var domain = this.app.getDomain()
  this.entries['muguet.' + domain] = this.app.getProxyIp()

  var self = this

  _.forEach(entries, function(names, ip) {
    names.forEach(function(name) {
      var alias = name.replace('.' + domain, '')
      self.entries[alias] = self.entries[name] = ip
      Logger.info(util.format('Resolving domain %s/%s to IP %s', name.yellow, alias.yellow, ip.yellow))
    })
  })
}
